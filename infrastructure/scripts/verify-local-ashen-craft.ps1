...
try {
    ...
    foreach ($id in @(9500, 9501, 9523, 9545, 9546, 9547, 9568, 9572, 9573, 9594)) {
        if ($items -notmatch "item id=`"$id`"") {
            throw "Ashen craft scroll id $id was not found."
        }
    }
    foreach ($reserved in @(9569, 9570, 9571)) {
        if ($items -match "item id=`"$reserved`"") {
            throw "Craft scroll file must not redefine reserved id $reserved."
        }
    }
    ...
    foreach ($id in @(872, 873, 895, 917, 918, 940, 941, 942, 963)) {
        if ($recipes -notmatch "item id=`"$id`" recipeId=") {
            throw "Ashen recipe list id $id was not found in Recipes.xml."
        }
    }
    if ($recipes -notmatch 'ingredient id="9399"' -or $recipes -notmatch 'ingredient id="9499"' -or $recipes -notmatch 'ingredient id="9699"' -or $recipes -notmatch 'ingredient id="9799"') {
        throw 'Ashen fragment ingredients 9399/9499/9699/9799 were not found in recipes.'
    }
    ...
    Write-Host 'Scrolls 9500-9594 (skip 9569-9571) and Recipes.xml merge present; submodule clean.'
}
catch {
